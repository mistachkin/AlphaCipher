...
namespace eval ::Eagle::Common::Tools {
  #############################################################################
  #***************************** TOOL PROCEDURES ******************************
  #############################################################################
  ...
  proc getClockBuildNumber {} {
    ...
    #
    set today [clock scan "[clock format $now -format {%Y-%m-%d}] 00:00:00"]
...

  proc getFossilManifestInfo { directory } {
    ...
    #       ID.
    #
    set pattern {^[0-9a-f]{40}$}

    if {![regexp -line -- $pattern $id]} then {
      return [list]
    }
...

  proc requireEagleLibrary { toolPath } {
    ...
    #       directory.
    #
    set rootPath [file dirname $toolPath]

    #
    # NOTE: Figure out the location of the Eagle script library files.
    #
    set libPath [file join $rootPath Eagle lib Eagle1.0]
...

  proc tagPkgVersion { toolPath path } {
    ...
    if {[info exists id] && [info exists timeStamp]} then {
      ...
      #
      set pattern1 {SOURCE_ID\t\t".*?"}
      set pattern2 {SOURCE_TIMESTAMP\t".*?"}
